...
The README and DEVELOPMENT_LOG over-claim. The code is more modest. Productization starts from the code, not the badges.

| Claim | What the code actually does |
|-------|-----------------------------|
| "EMG Phase B+C operational" | Graph matching, insight compiler, retriever, and promotion **exist**. `InMemoryInsightStore`, `TaskMemoryNodeStore`, `CrossTaskEdgeStore` are **process-local dicts**. Restart the CLI or the API server and the knowledge is gone. |
| "Seed corpus populated" | `packages/seed-corpus/` has Python pipelines and YAML catalogs (`failure-modes.yaml`, `negative-knowledge.yaml`). There is **no** `artifacts/` directory and **no** `learning-sessions/` directory on disk. Tests construct stores in RAM. |
| "TF-IDF is fine until retrieval is a bottleneck" (WP-07 §14) | Retrieval **is** the product. `TfidfEmbedder` is a fixed ~60-term bag over OIW vocabulary. It cannot match paraphrases. `FastembedEmbedder` (MiniLM) exists in `apps/cli/oiw/emg/embedding.py` but `oiw[embeddings]` is **not declared** in `apps/cli/pyproject.toml`, so auto-select falls through to TF-IDF. |
| "Real SAP CI adapter (OW-010)" | `SapCiTenantAdapter` implements list/download/upload/deploy against `/api/v1`. It is **not** the default. `build_tenant_adapter()` returns `MockSapCiTenantAdapter` unless `OIW_USE_REAL_TENANT=1`. Adapter tests use `httpx.MockTransport`. No live tenant job exists. |
| "CodeJam ingested" | One export ZIP lives at `packages/test-fixtures/real-sap/sap-codejam-request-employee-dependants/`. Import status is **PARTIAL**. Five `callActivity` steps are `unsupported`. WP-07 Task A-001 (≥ 8 artifacts) is still unchecked. `real_ingestion.py` **invents a minimal HTTP sender flow** when the parser fails — that is synthetic knowledge labeled as CodeJam. |
| "Web UI is substantially complete" | `apps/web/src/App.tsx` is a 552-line god component. EMG panel reads `/api/v1/projects/{id}/emg/insights`, which is a module-level dict populated **only in tests** (`populate_emg_api`). Server startup in `oiw_server/main.py` never loads a corpus. Empty panel is the real UX. |
| "`oiw learn` CLI" (WP-07) | Learning session dataclasses exist under `apps/cli/oiw/learn/`. **No** `oiw learn` Click group is registered in `apps/cli/oiw/cli.py`. Only `oiw emg report` and `oiw emg provenance` exist, and they audit YAML catalogs, not a live store. |
...
### Circularity trap (do not repeat)

WP-07's own warning: if the import parser cannot handle a real iFlow, `create_pattern_from_analysis()` in `packages/seed-corpus/real_ingestion.py` fabricates a sender.http → (optional validator/script/xslt) → receiver.http skeleton. Promoting that as `provenance.source = "sap-codejam"` teaches the EMG OIW's guesses, not SAP's artifact. **Failed imports are parser bugs or documented gaps. They are not expert trajectories.**
