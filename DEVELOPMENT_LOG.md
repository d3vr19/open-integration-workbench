...
### 2026-09-03 (cont. 4) — B-2 COMPLETE: Saxon-HE XSLT2 bridge live; pieces must be SHIPPABLE; Hub-harvest scope banked

**Operator unblocked the environment:** full JDK 21 installed (javac live after `source /etc/profile`); GroovyRunner compiled and verified. And corrected my misreading of "discover": the **Business Accelerator Hub** (Discover tab on the tenant) is a THIRD-PARTY package library — copy-to-tenant gives us reference shapes for ANY CPI component family, far beyond our own 600 flows. That converts every remaining exporter gap (validator, encoder-encode-direction, splitter, gather, router, Mapping variants) from "hunt for luck on our tenant" to "pick the best-maintained package and harvest". Hub-harvest = the new primary METHOD step for reference acquisition; UI copy actions are the operator's (or the flows arrive already-copied for me to pull).

**Shipped (B-2):**
- **XsltRunner.java** (services/runtime-worker-jvm/src/io/oiw/xslt/) — Saxon-HE 12.5 subprocess bridge, protocol mirrors GroovyRunner (stdin/stdout JSON, base64 body, ExecutorService timeout, no network fetches); xmlresolver 5.2.2 added (Saxon 12.x runtime dep — NoClassDefFoundError without it). setup.sh compiles both runners; `oiw-xslt-runner.sh` wrapper. Live-verified: XSLT2 `upper-case()` transforms (impossible in lxml).
- **xslt_transform.py rework** — Saxon-first, lxml-XSLT1 fallback (bridge-absent environments, e.g. CI, stay honestly `simulated` via DYNAMIC fidelity declaration); **order-to-s4 happy-path local-PASS in real engine** — the case that sat UNSUPPORTED since P5a now runs.
- **THE SHIPPABLE-PIECE LAW (live lesson):** a turbo piece must be BOTH runtime-real AND exporter-renderable (`_shippable_pieces()` = intersection; endpoints always renderable via messageFlow shapes). transform.xslt is now runtime-real but has no Mapping activityType — NOT a piece. splitter/gather (real since H4/H5) similarly wait on exporter shapes. Tests updated to the new truth; his H8 splitter e2e now asserts the honest escalation (unmatched_components = the teacher signal, never silent drop).
